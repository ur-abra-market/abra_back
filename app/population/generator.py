...
class DataGen:
    ...
    def _load_constants(self):
        ...
        self.suppliers_ids = loop.run_until_complete(self.get_existing_ids(orm.suppliers))
        self.cat_ids = loop.run_until_complete(self.get_existing_ids(orm.categories))
        self.sellers_ids = loop.run_until_complete(self.get_existing_ids(orm.sellers))
        self.order_status_ids = loop.run_until_complete(self.get_existing_ids(orm.orders_statuses))
        ...
        for _ in range(how_many):
            async with async_sessionmaker.begin() as session:
                product: ProductModel = await orm.products.insert_one(
                    session=session,
                    values={
                        ProductModel.name: faker.sentence(nb_words=2),
                        ...
                        ProductModel.supplier_id: random.choice(self.suppliers_ids),
            ...
            async with async_sessionmaker.begin() as session:
                order = await orm.orders.insert_one(
                    ...
                    values={
                        ...
                        OrderModel.seller_id: random.choice(self.sellers_ids),
                        OrderModel.status_id: random.choice(self.order_status_ids),
                    },
                )
                logger.info(f"загрузили {order}")

    async def _generate_users(self, how_many: int = 10):
            ...
            async with async_sessionmaker.begin() as session:
                ...
                await orm.users_credentials.insert_one(
                    ...
                    values={
                        ...
                        UserCredentialsModel.password: hash_password("Password1!"),
                    },
                )
                if user.is_supplier:
            ...
            async with async_sessionmaker.begin() as session:
                await orm.companies.insert_one(
                    ...
                    values={
                        ...
                        CompanyModel.supplier_id: random.choice(self.suppliers_ids),
    ...
    def load_products(self):
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self._generate_products_with_prices())

    def load_users(self):
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self._generate_users())
