...
class SellerAddressSchema(BaseModel):
    country: str = None
    area: str = None
    city: str = None
    street: str = None
...
@sellers.get("/get_seller_info/", summary="WORKS: returns dict with profile info,")
async def get_seller_info(
    ...
    user_addresses = [
        dict(
            ...
            area=user_address[0].area,
            city=user_address[0].city,
            street=user_address[0].street,
            building=user_address[0].building,
            appartment=user_address[0].appartment,
...
async def add_seller_address(
    seller_address_data: SellerAddressSchema,
    Authorize: AuthJWT = Depends(),
    session: AsyncSession = Depends(get_session),
):
    Authorize.jwt_required()
    ...
    user_address = SellerAddress(**seller_address_data.dict(), seller_id=seller_id)
...
@sellers.patch(
    "/update_addresses/",
    response_model=SellerAddressSchema,
    response_model_exclude_unset=True,
)
...
@sellers.get("/addresses/", response_model=SellerAddressSchema)
    ...
    seller_address = (
        await session.execute(
            select(
                ...
                SellerAddress.building,
                SellerAddress.appartment,
                SellerAddress.postal_code,
            ).where(SellerAddress.seller_id.__eq__(seller_id))
        )
    ).all()
