...
async def register_user(
    ...
    if user_type not in ["sellers", "suppliers"]:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="INCORRECT_SUBDOMAIN")
    ...
    if is_email_unique:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="EMAIL_EXISTS")

    is_supplier = 1 if user_type == "suppliers" else 0
    current_datetime = utils.get_moscow_datetime()
    user = User(email=user_data.email, datetime=current_datetime, is_supplier=is_supplier)
    session.add(user)
