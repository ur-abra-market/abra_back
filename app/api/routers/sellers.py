...
@sellers.get(
    "/get_seller_info/",
    summary="WORKS: returns dict with profile info," "adresses, notifications, photo information",
)
...
async def send_seller_data_info(
    ...
    user_data: Optional[dict] = (
        {key: value for key, value in dict(seller_data).items() if value} if seller_data else None
    )
    ...
    if address_data:
        await session.execute(
            update(UserAdress).where(UserAdress.user_id.__eq__(user_id)).values(**address_data)
        )
...
async def add_seller_address(
    ...
    return JSONResponse(
        status_code=status.HTTP_201_CREATED, content={"result": {"address_id": user_address.id}}
    )


@sellers.patch(
    "/update_addresses/", response_model=SellerUserAddressSchema, response_model_exclude_unset=True
)
async def change_seller_address(
    ...
    if seller_id is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="USER_NOT_FOUND")
    seller_address_data = {
        key: value for key, value in dict(seller_address_data).items() if value is not None
    }
    await session.execute(
        update(UserAdress)
        .values(dict(seller_address_data))
        .where(and_(UserAdress.id.__eq__(address_id), UserAdress.user_id.__eq__(seller_id)))
    )
    await session.commit()

    return JSONResponse(status_code=status.HTTP_200_OK, content={"result": "ADDRESS_WAS_UPDATED"})
...
async def get_seller_addresses(
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK, content={"result": {"seller_address": seller_address}}
    )


@sellers.delete("/remove_addresses/{address_id}")
async def remove_seller_address(
    address_id: int, Authorize: AuthJWT = Depends(), session: AsyncSession = Depends(get_session)
):
    Authorize.jwt_required()
    user_email = json.loads(Authorize.get_jwt_subject())["email"]
    seller_id = await Seller.get_seller_id_by_email(user_email)
    ...
    await session.commit()
    return JSONResponse(status_code=status.HTTP_200_OK, content={"result": "ADDRESS_WAS_DELETED"})
